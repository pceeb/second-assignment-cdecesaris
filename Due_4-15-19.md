...
1. What is the procedure for logging into Hoffman2?
	a) using a secure shell, remote connect to the hoffman via  user-0i@hoffman2.idre.ucla.edu  cluster or enter username seperatly if you use Putty
	b)enter password to access the login node
2. What is the first thing that you do once you login?  What is the command?
 	a) get out of the login node by requesting another node with the qrsh command. Specify the amount of data needed otherwise default will be set to 1G
___
...
1. You can use two flags at the same time. What does the command ls do when used with the -l flag? What about if you use both the -l and the -h flag?
	The -l flag will list the root, date created, size and names  of all items in the current directory. A command -l -h will list the same thing except larger data numbers will be written in short notation. e.g. 4.0K vs 4000.0 
...
2. The command ls -R lists the contents of directories recursively, i.e., lists their sub-directories, sub-sub-directories, and so on at each level. The command ls -t lists things by time of last change, with most recently changed files or directories first. In what order does ls -R -t display things? Hint: ls -l uses a long listing format to view timestamps.
 	The command ls -R -t lists the recursive contents with the last changed item in that directory first and so on. The list format is in rows as opposed to columns.
---

### Shortcuts

1. what is ~  a) represents the home directory the command cd ~ will take you home.
2. what is ../  a) represents the address of directory proceeding the current directory. cd ../ will take you back a directory.
3. what is ./? a) represents the current directory.
...
Created a new text file which does appear. It is empty and has zero bytes.

 When might you want to create a file this way?
If you wish to mkae a file without yet modifying or adding something to it.

 Can you use touch to edit my_file.txt?
No you cannot use touch to modify files.
...
The >> appends $ echo [input] to the end of the file.
The > overwrites the $ echo [input] into the file—deleting everything previously in the text file. It will create a new file if this doesn’t exist.
...
```
  ...
  d. The second and third lines of animals.txt  CORRECT
...
```
$ mv ../analyzed/sucrose.dat ../analyzed/maltose.dat ._
```
...
4. What happens when we execute `rm -i thesis_backup/quotations.txt`? Why would we want this protection when using rm?
  This removes the files but prompts the user first for agreement as a safty measure because when something is deleted it is deleted forever.
     This copy's both of them into the directory.
5. In the example below, what does cp do when given several filenames and a directory name?
``` In my case, it still copied all the named files into the directory.
$ mkdir backup
$ cp amino-acids.txt animals.txt backup/
```

6. In the example below, what does cp do when given three or more file names?
``` In my case, the three files were copied into the directory.
$ ls -F
```
...
$ cp *_calibration*  backup/calibration
...
$ mkdir 2016-05-20/data/processed
   --This one is CORRECT(previous one)
...
It is sorting the numbers from least to greatest via the -n tag.
...
Datasets often contain duplicate data which is repeated and needs to be removed if it is not significant.
...
File would contain lines 5, 4 & 3 in this order
...
```
rm ?.txt   Removes all te one charavter text files.
rm \*.txt CORRECT answer
rm \* .txt Removes all the matching files with " " and a file named .txt
